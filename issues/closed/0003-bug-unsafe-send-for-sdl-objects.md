...
- `Renderer` と `Window` から `unsafe impl Send` を削除した。
- `PhantomData<MutexGuard<'static, ()>>` により型を `!Send` にし、SDL のウィンドウ／レンダラのスレッドアフィニティと矛盾しない契約にした。
- これに伴い `VideoPlayerInner` を含む `Mutex<VideoPlayerInner>` は `Send` を満たさず、`VideoPlayer` は `Send` / `Sync` を実装しなくなった。
