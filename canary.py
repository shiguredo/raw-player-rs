...
# cargo update raw_player を実行
def run_cargo_update(dry_run: bool) -> None:
    if dry_run:
        print("Dry-run: Would run 'cargo update raw_player'")
    else:
        subprocess.run(["cargo", "update", "raw_player"], check=True)
        print("cargo update raw_player executed")
